...
and eventually custom configurations. **None of those other modes are being
built yet.** This document exists so that when they are, it doesn't require
...
If you're implementing a later phase and it's not obvious whether something
belongs in the shared core or in service-business-specific code, read this
first.
...
- Nav items in `src/lib/nav.ts` (`Home`, `Customers`, `Jobs`, `Schedule`,
  `Team`, `Settings`): currently hardcoded English labels because
  `Customers`/`Jobs` don't exist as real features yet — they're
  placeholders. When they become real (Phase 2), that's when to introduce
  the label-lookup layer described above, not before.
